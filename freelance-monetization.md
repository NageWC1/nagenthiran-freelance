...
### Centering bug found and fixed — 2026-08-31
User manually cropped the right side off several flyer images because content looked shifted left, and asked for the root cause to be fixed rather than keep manually cropping. Investigated properly rather than guessing at CSS: confirmed via `getBoundingClientRect()` that the actual HTML/CSS layout is genuinely symmetric (every content element has identical left/right gaps), and confirmed the PDF's own `/MediaBox` is exact A4. The real bug: **Chrome's `--print-to-pdf` pipeline itself** — it shrinks page content to ~82% and anchors it top-left instead of centering it on the page canvas, a behavior specific to the print rendering path (reproduced identically under both `--headless` and `--headless=new`, so not a headless-mode difference). Measured directly: print-to-pdf output showed ~110px left margin vs. ~405px right margin (nearly 4x asymmetric); the same HTML rendered via a normal screenshot showed ~134px on both sides.

This means **every PDF generated in this repo via `--print-to-pdf` since the very first post has had this miscentering** — the "First pass was left-aligned... fixed by switching to A4" note in the very first status entry above was an incomplete fix; switching to A4 and centering the CSS didn't address this, because the bug was never in the CSS.

Fixed by changing the render pipeline entirely: screenshot the HTML directly (proven correct) at a device-scale-factor tuned for ~200dpi on A4, then build the PDF straight from that verified PNG via PIL (`Image.save(..., resolution=200.0)`) instead of Chrome's print pipeline. Regenerated all 6 posts' PDFs and PNGs this way, re-verified centering via direct pixel measurement on all 6 (left/right margins now match within a few px everywhere, vs. the previous ~4x asymmetry) and visually confirmed 2 of the 6. Full corrected workflow documented in `CLAUDE.md` with an explicit warning not to reintroduce `--print-to-pdf` for this repo.
